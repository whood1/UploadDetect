        ...
        try {
            while (i--) { // Run in reverse order
                ...
                if ((t.scope === "all" || t.scope === scope) && (!t.trigger || t.trigger.call(context))) {
                    ...
                    if (match && match.uploader_index === pos) {
                        result = {
 ...
 */
    self.exec = function (str, regex, pos, sticky) {
        ...
        if (sticky && match && match.uploader_index !== pos) {
            match = null;
        }
        if (regex.global) {
 ...
 */
    self.forEach = function (str, regex, callback, context) {
        ...
        while ((match = self.exec(str, regex, pos))) {
            ...
            pos = match.uploader_index + (match[0].length || 1);
        }
        return context;
    };
 ...
 */
    fixed.split = function (separator, limit) {
        ...
        self.forEach(str, separator, function (match) {
            if ((match.uploader_index + match[0].length) > lastLastIndex) { // != `if (match[0].length)`
                output.push(str.slice(lastLastIndex, match.uploader_index));
                if (match.length > 1 && match.uploader_index < str.length) {
                    Array.prototype.push.apply(output, match.slice(1));
                }
                lastLength = match[0].length;
                lastLastIndex = match.uploader_index + lastLength;
            }
        });
 ...
 */
    add(/(?:\(\?#[^)]*\))+/,
        function (match) {
            ...
            return nativ.test.call(quantifier, match.input.slice(match.uploader_index + match[0].length)) ? "" : "(?:)";
        });
    ...
    add(/\\k<([\w$]+)>/,
        function (match) {
            var index = isNaN(match[1]) ? (lastIndexOf(this.captureNames, match[1]) + 1) : +match[1],
                endIndex = match.uploader_index + match[0].length;
    ...
    XRegExp.matchRecursive = function (str, left, right, flags, options) {
        ...
        while (true) {
            ...
            if (leftMatch && rightMatch) {
                if (leftMatch.uploader_index <= rightMatch.uploader_index) {
                    rightMatch = null;
                } else {
                    leftMatch = null;
                }
            ...
            * Loop ends after the first completed match if `!global` */
            if (leftMatch || rightMatch) {
                delimStart = (leftMatch || rightMatch).uploader_index;
